...
def raytrace_ana(a=SPIN,
                 ...
                 verbosity=0):
    """
    Raytrace geodesics in Kerr using the analytic formalism of Gralla+Lupsasa 2019a,b.

    Parameters
    ----------
    a (float) : black hole spin in range [0, 1)
    observer_coords (list) : [t_o, r_o, th_o, ph_o] Boyer-Lindquist observer coordinates
    image_coords (list) : [alpha, beta] Bardeen image plane coordinates; alpha & beta may be arrays
    ngeo (int) : number of steps to evaluate along geodesic
    do_phi_and_t (bool) : whether to calculate phi and t along geodesic
    savedata (bool) : whether to save data to file
    plotdata (bool) : whether to display a plot of geodesics
    verbose (int) : verbosity level (higher is more verbose)

    Returns
    -------
    geos : Geodesics object, can access geodesics with geos.geo_coords

    TODOs
    -----
    - ererors in phi raytracing with alpha == 0, beta != 0
    """
    ...
    if th_o > np.pi/2.: 
        print("WARNING th_o>pi/2! Not fully verified!")
    ...
    start = time.time()
    (th_s, G_ph, G_t) = th_integrate(a, th_o, s_o, lam, eta, u_plus, u_minus,
                                     tausteps, do_phi_and_t=do_phi_and_t)
    stop = time.time()
    if verbosity > 0:
    ...
    start = time.time()
    (r_s, I_ph, I_t, I_sig) = r_integrate(a, r_o, lam, eta, r1, r2, r3, r4,
                                          tausteps, do_phi_and_t=do_phi_and_t)
    stop = time.time()
    if verbosity > 0:
...
def r_integrate(a, r_o, lam, eta, r1, r2, r3, r4, tausteps,
    ...
    if not isinstance(r3, np.ndarray):
        r3 = np.array([r3]).flatten()
    if not isinstance(r4, np.ndarray):
    ...
    # TODO what to do if we are exactly on the critical curves of double roots
    # between regions?
...
def S1_S2(al, phi, j, ret_s2=True):  # B92 and B93 of GL19a
    ...
    # definitions of functions in this region should keep things in the
    # allowed prange
